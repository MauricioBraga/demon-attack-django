...
export const CACHE_NAME = 'demon-attack-cache-2026-09-06';
...
self.addEventListener('fetch', e => {
    ...
    // Dados dinâmicos (recordes, ranking): sempre buscar da rede, nunca
    // servir uma resposta em cache. Do contrário, a primeira leitura de
    // cada endpoint ficaria congelada em cache para sempre — o placar
    // nunca mostraria recordes novos depois da primeira visita.
    if (new URL(e.request.url).pathname.startsWith('/api/')) {
        e.respondWith(
            fetch(e.request).catch(() => new Response('Service Unavailable', { status: 503 }))
        );
        return;
    }
