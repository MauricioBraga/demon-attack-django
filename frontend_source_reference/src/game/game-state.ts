import { Demon } from './demon';
import { Cannon } from './cannon';
import { CannonBullet } from './cannon-bullet';
import { DemonBullet } from './demon-bullet';
import { store, saveStore } from '@/store';

const CANNON_FIRING_SPEEDS = [ 3, 3, 4, 4, 5, 5, 5, 5, 6 ];
const DEMON_FIRING_SPEEDS = [ 8, 6, 6, 3, 5, 4 ];

export class GameState {
    level = 0;
    demonPalette = 0;
    demonType = 0;
    backgroundColor = 0;
    baseColor = 0;
    bunkers: number;
    bunkerColor = 0;
    score: number;
    scoreColor = 0x2C;
    cannon = new Cannon();
    cannonBullet = new CannonBullet();
    cannonFiringSpeed = 0;
    demons = new Array<Demon>();
    spawnDelay = 0;
    spawnedDemons: number;
    divingDemon: Demon | null = null;
    demonBullets = new Array<DemonBullet>();
    demonBulletDropTimer = 0;
    demonBulletDropTimerReset = 0;
    animatingExtraBunker = false;
    animatingGameOver = false;
    newHighScore = false;
    pulseCounter = 0;
    demo: boolean;

    constructor(demo: boolean = false) {
        this.demo = demo;
        // Toda partida — real ou demo — sempre começa do zero. Não existe
        // mais retomada de uma sessão anterior: nem entre recarregamentos de
        // página, nem entre reinícios do servidor (esse estado nunca chega a
        // ser salvo no backend Django, apenas o resultado final da partida,
        // via /api/scores/). Score, nível, bunkers e demônios já spawnados
        // são sempre reiniciados.
        this.setLevel(0);
        this.score = 0;
        this.spawnedDemons = 0;
        this.bunkers = 3;
        this.cannon.exploded = false;
    }

    private setLevel(level: number) {
        this.level = level;
        this.demonPalette = level % 7;
        this.demonType = (level >> 1) % 6;
        this.cannonFiringSpeed = store.autofire ? 12 : CANNON_FIRING_SPEEDS[Math.min(8, level)];
        this.demonBulletDropTimerReset = DEMON_FIRING_SPEEDS[(level < 4) ? level : (level & 1) + 4];
        this.spawnDelay = 30;
        this.demonBulletDropTimer = 0;
        this.spawnedDemons = 0;
    }

    incrementLevel() {
        this.setLevel(this.level + 1);        
    }

    save() {
        // Não há mais retomada de partida entre sessões (ver constructor
        // acima) — isso só garante que preferências como high score, volume
        // e as opções (autofire/tracer/fast) continuem salvas no navegador.
        saveStore();
    }

    removeDemon(demon: Demon) {
        if (this.divingDemon == demon) {
            this.divingDemon = null;
        }
        if (demon.partner) {
            demon.partner.partner = null;
        }
        for (let i = this.demons.length - 1; i >= 0; --i) {
            if (this.demons[i] === demon) {
                this.demons.splice(i, 1);
                return;
            }
        }
    }
}