...
export function resetGame(demo: boolean = false) {
    gs = new GameState(demo);
...
function trySpawnDemon() {
    ...
    if (tier !== Tier.NONE && yMax - yMin + 1 >= 8 && gs.spawnedDemons < 8) {        
        ...
        if (!gs.demo) {
            // Em modo demo, nada deve chamar play() antes da primeira interação
            // real do usuário: o navegador bloqueia audioContext.resume() sem um
            // gesto do usuário, e o helper resume() (audio.ts) cacheia essa
            // primeira tentativa indefinidamente — travando o áudio pelo resto
            // da sessão mesmo depois do jogo real começar.
...
export function update() {
    ...
    if (gs.demo) {
        // Tela de atração: pressionar o botão de tiro (espaço, clique/toque
        // ou qualquer outra tecla — ver src/input.ts) encerra a demonstração
        // e inicia uma partida real. As setas não fazem nada aqui, para não
        // iniciar o jogo sem querer enquanto o usuário só está explorando.
        if (isFirePressed()) {
            resetGame(false);
            return;
        }
        // Os demônios surgem e se movem exatamente como no jogo real, mas o
        // canhão fica parado (cannon.ts/cannon-bullet.ts ignoram a entrada
        // do usuário quando gs.demo é true) e nenhum tiro é trocado, então a
        // demonstração roda indefinidamente sem nunca terminar a partida.
        if (gs.spawnedDemons === 8 && demons.length === 0) {
            gs.incrementLevel();
        }
        trySpawnDemon();
        for (let i = demons.length - 1; i >= 0; --i) {
        ...
        // Nenhum som toca no modo demo (ver comentário em trySpawnDemon()
        // sobre autoplay/gesto do usuário), então o pulso cardíaco fica
        // silencioso aqui — volta a soar normalmente assim que o jogo real começa.
        return;
    }
