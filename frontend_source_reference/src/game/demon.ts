...
for (let level = 0; level < 12; ++level) {
    EXPLODE_SFXS[level] = `sfx/explodes-demon-${level.toString().padStart(2, '0')}.mp3`;
}
    ...
    update(gs: GameState) {
        ...
        if (this.spawning > 0) {
            ...
            if (cannonBullet.state === CannonBulletState.FIRING && bulletIntersects(cannonBullet.x, cannonBullet.y, 
                        store.autofire ? 12 : 8, this.split ? splitDemonMasks[this.sprite] 
        ...
        if (this.tier === Tier.DIVING) {
            ...
            if (this.split && !this.leftHalf) {
                ...
                if (this.xEaser.update()) {
                    if (!gs.demo && this.tier === Tier.BOTTOM && !this.exploding && this.spawning === 0 
